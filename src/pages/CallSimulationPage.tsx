...
import { Mic, MicOff, PhoneOff, Globe, Volume2, User, MessageCircle, Network, ChevronLeft, Speaker, Disc, LayoutList, GripVertical, Hash, Info } from "lucide-react";
...
export default function CallSimulationPage({ onCallStateChange }: CallSimulationPageProps) {
  ...
  const { query, sources: knowledgeSources, loading: isKnowledgeLoading } = useKnowledgeAssistant();
                      ...
                      {messages.map((m, i) => (
                        ...
                        >
                          ...
                          <div className={cn(
                            ...
                            {m.content}
                            {m.role === 'assistant' && i === messages.length - 1 && knowledgeSources.length > 0 && (
                              <SourceAttribution sources={knowledgeSources} />
                            )}
                          </div>
                        </motion.div>
                      ))}
                      {(isThinking || isKnowledgeLoading) && (
